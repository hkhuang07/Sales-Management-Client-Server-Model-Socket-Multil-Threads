        ...
        private async Task LogIn()
        {
            ...
            while (!isAuthenticated)
            {
                ...
                if (logIn.ShowDialog() == DialogResult.OK)
                {
                    ...
                    {
                        employeeName = !string.IsNullOrEmpty(loginResponse.FullName) ? loginResponse.FullName : loginResponse.Username;
                        userID = loginResponse.UserId;

                        if (userID == 0 && !string.IsNullOrEmpty(employeeName))
                        {
                            EmployeeDTO user = await _clientService.GetEmployeeByNameAsync(employeeName);
                            if (user != null)
                            {
                                userID = user.ID;
                            }
