    ...
    public class ServerHandler 
    {
        ...
        public static async Task HandleClientAsync(TcpClient client)
        {
            ...
            try
            {
                ...
                // Đảm bảo _mapper đã được khởi tạo trước khi gọi HandleClientAsync
                if (_mapper == null)
                {
                ...
                while ((bytesRead = await stream.ReadAsync(buffer, 0, buffer.Length)) != 0)
                {
                    ...
                    {
                        responseBase.Message = $"Error parsing request: {ex.Message}";
                        await SendResponse(stream, responseBase);
                        continue;
                    }
                    ...
                    {
                        responseBase.Message = $"JSON deserialization error for method '{requestBase.MethodName}': {jsEx.Message}. Check the data format.";
                        Console.WriteLine($"JSON Deserialization Error: {jsEx.Message} | Request Data: {requestBase.Data}");
                    }
                    catch (ArgumentException aEx)
                    {
                        responseBase.Message = $"Validation Error for method '{requestBase.MethodName}': {aEx.Message}";
                        Console.WriteLine($"Argument Error: {aEx.Message}");
                    }
                    catch (Exception ex)
                    {
                        responseBase.Message = $"Server error processing method '{requestBase.MethodName}': {ex.Message}";
                        Console.Error.WriteLine($"Error in HandleClientAsync (Method: {requestBase.MethodName}): {ex}");
                    }
                    finally
                    {
                        await SendResponse(stream, responseBase);
